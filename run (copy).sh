...
function generateVolumeMapping() {
    if [ "$VOLUMES_LIST" == "" ]; then
        ...
        VOLUMES_LIST=`cat ${DOCKER_ENV}|grep "^#VOLUMES_LIST= *"|sed "s/[#\"]//g"|cut -d'=' -f2-`
    fi
...
function generatePortMapping() {
    if [ "$PORTS" == "" ]; then
        ## -- If locally defined in this file, then respect that first.
        ## -- Otherwise, go lookup the docker.env as ride-along source for volume definitions
        PORTS_LIST=`cat ${DOCKER_ENV}|grep "^#PORTS_LIST= *"|sed "s/[#\"]//g"|cut -d'=' -f2-`
    fi
...
replaceKeyValue ./${DOCKER_ENV} "USER_ID" "$(id -u $USER)"
replaceKeyValue ./${DOCKER_ENV} "GROUP_ID" "$(id -g $USER)"
